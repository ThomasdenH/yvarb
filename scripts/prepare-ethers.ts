...
import deployment from '../frontend/src/generated/deployment.json';
...
async function grantPermission() {
    const yieldLeverAddress = deployment.deployedTo;
    await provider.send('evm_setAccountBalance', [TimeLock.address, '0x38D7EA4C680000000']);
    const iface = new ethers.utils.Interface(['function give(bytes12 amount, address dest)']);
    const sig = iface.getSighash('give');
    const timelockSigner = provider.getSigner(TimeLock.address);
    const Cauldron = new Contract('0xc88191F8cb8e6D4a668B047c1C8503432c3Ca867', cauldronAbi, timelockSigner);
    await Cauldron.grantRole(sig, yieldLeverAddress);
    console.log('Granted YieldLever access.');
}
