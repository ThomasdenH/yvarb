...
export async function loadVaults(cauldron: Contract, account: string, provider: ethers.providers.Web3Provider) {
    console.log('call');
    const currentBlock: number = await provider.getBlockNumber();
    const vaultsBuiltFilter = cauldron.filters.VaultBuilt(null, account, null);
    const vaultsReceivedFilter = cauldron.filters.VaultGiven(null, account);
    for (let start = CAULDRON_CREATED_BLOCK_NUMBER; start < currentBlock; start += BLOCK_STEPS) {
        const end = Math.min(start + BLOCK_STEPS, currentBlock);
        console.log(start, end);
        console.log((start - CAULDRON_CREATED_BLOCK_NUMBER) / (currentBlock - CAULDRON_CREATED_BLOCK_NUMBER));
