...
export default class UsdcInput extends React.Component<Props, State> {
    constructor(props: Props) {
        super(props);
        this.state = { value: utils.formatUnits(props.defaultValue, 6), focus: false, prettyValue: formatUSDC(props.defaultValue) };
    }

    render(): React.ReactNode {
        const value = (!this.state.focus && this.state.prettyValue !== undefined) ? this.state.prettyValue : this.state.value;
    ...
    private onChange(value: string) {
        if (this.state.focus) {
            ...
            const prettyValue = parsedValue === undefined ? undefined : formatUSDC(parsedValue);
